...
ENVEOF
  ...
  echo "      ⚠️  Edit $ENV_FILE with your credentials before starting the service!"
else
...
# ── 7b. Install Foundry (native ARM64 Solidity compiler) ─────────────────────
echo "[7b] Installing Foundry (forge) for ARM64-native contract compilation..."
if ! command -v forge &>/dev/null; then
  curl -fsSL https://foundry.paradigm.xyz | bash -s -- --no-modify-path
  export PATH="$HOME/.foundry/bin:$PATH"
  foundryup &>/dev/null
  echo "     forge installed: $(forge --version 2>/dev/null || echo reopen shell && run foundryup)"
else
  echo "     forge already present: $(forge --version)"
fi
echo "     Add to PATH if needed: export PATH=\$HOME/.foundry/bin:\$PATH"
